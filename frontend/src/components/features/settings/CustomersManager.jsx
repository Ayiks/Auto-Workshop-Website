...
export default function CustomersManager() {
  ...
  return (
    <Card className="bg-white border border-gray-200 shadow-sm overflow-hidden">
      {/* Header */}
      <div className="px-4 sm:px-6 py-3 sm:py-5 border-b border-gray-100 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 sm:gap-4 bg-gray-50/50">
        <div>
          <h2 className="text-sm sm:text-base font-bold text-gray-900">Customer Database</h2>
          <p className="text-xs text-gray-500 mt-0.5">Manage customer contacts and details</p>
        </div>
        
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 w-full sm:w-auto">
            <div className="relative flex-1 sm:w-64">
                <svg className="absolute left-2.5 sm:left-3 top-1/2 -translate-y-1/2 w-3.5 sm:w-4 h-3.5 sm:h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                <input
                    type="text"
                    placeholder="Search customers..."
                    className="w-full pl-8 sm:pl-9 pr-3 sm:pr-4 py-1.5 sm:py-2 text-xs sm:text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-gray-900 focus:border-gray-900"
                    value={searchTerm}
            ...
            {canEdit && (
                <Button 
                    ...
                    className="bg-gray-900 text-white hover:bg-black flex-shrink-0 whitespace-nowrap"
                >
      ...
      {customers.length === 0 ? (
        <div className="text-center py-8 sm:py-12 md:py-16 px-4">
            ...
            <h3 className="text-xs sm:text-sm font-medium text-gray-900">No customers found</h3>
            <p className="text-xs sm:text-sm text-gray-500 mt-1">Try adjusting your search terms or add a new customer.</p>
        </div>
      ) : (
            ...
            <table className="w-full text-left border-collapse">
                <thead>
                    <tr className="border-b border-gray-100 bg-gray-50/50">
                        <th className="px-3 sm:px-4 py-2 sm:py-3 text-[10px] sm:text-xs font-semibold text-gray-500 uppercase tracking-wider">Name / Address</th>
                        <th className="px-3 sm:px-4 py-2 sm:py-3 text-[10px] sm:text-xs font-semibold text-gray-500 uppercase tracking-wider">Contact Info</th>
                        <th className="px-3 sm:px-4 py-2 sm:py-3 text-[10px] sm:text-xs font-semibold text-gray-500 uppercase tracking-wider">Notes</th>
                        {canEdit && <th className="px-3 sm:px-4 py-2 sm:py-3 text-[10px] sm:text-xs font-semibold text-gray-500 uppercase tracking-wider text-right">Actions</th>}
                    </tr>
                </thead>
                        ...
                        <tr key={customer.id} className="hover:bg-gray-50 transition-colors group">
                            <td className="px-3 sm:px-4 py-2 sm:py-3">
                                <div className="flex flex-col">
                                    <span className="text-xs sm:text-sm font-medium text-gray-900">{customer.firstName} {customer.lastName}</span>
                                    <span className="text-[10px] sm:text-xs text-gray-500 truncate max-w-[200px]">{customer.address || '-'}</span>
                                </div>
                            </td>
                            <td className="px-3 sm:px-4 py-2 sm:py-3">
                                <div className="flex flex-col gap-0.5 sm:gap-1">
                                    <div className="flex items-center text-xs sm:text-sm text-gray-700">
                                        <svg className="w-3 h-3 mr-1.5 sm:mr-2 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                                        {customer.phone}
                                    </div>
                                    {customer.email && (
                                        <div className="flex items-center text-[10px] sm:text-xs text-gray-500">
                                            <svg className="w-3 h-3 mr-1.5 sm:mr-2 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                                            {customer.email}
                                        </div>
                                    )}
                                </div>
                            ...
                            <td className="px-3 sm:px-4 py-2 sm:py-3">
                                <span className="text-xs sm:text-sm text-gray-500 italic block truncate max-w-[200px]">
                                    {customer.notes || '-'}
                                </span>
                            </td>
                            {canEdit && (
