...
export default function Materials() {
  ...
  const inventoryStats = useMemo(() => {
    if (!materials || materials.length === 0) {
      return { totalItems: 0, lowStockItems: 0, totalCostValue: 0, totalRetailValue: 0, activeItems: 0 };
    }
  ...
  const columns = [
        ...
        return (
          <input
            ...
            className={`rounded border-gray-300 text-gray-900 focus:ring-gray-900 h-4 w-4 transition-all ${
              isInactive ? 'opacity-20 cursor-not-allowed bg-gray-100' : 'cursor-pointer'
            }`}
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className={`font-semibold text-sm ${row.isActive ? 'text-gray-900' : 'text-gray-400 line-through'}`}>
              {row.name}
            </p>
    ...
    },
    {
      ...
      render: (row) => {
        ...
        const percentage = threshold > 0 ? Math.min((qty / (threshold * 3)) * 100, 100) : 100;

        return (
          <div className="w-full max-w-[120px]">
            <div className="flex justify-between text-xs mb-1">
              <span className={`font-medium ${isLow ? 'text-red-600' : 'text-gray-900'}`}>{qty}</span>
    ...
    },
    {
      header: "Cost",
      accessor: "unitCost",
      render: (row) => <span className="text-sm text-gray-500 font-mono">GH₵{Number(row.unitCost || 0).toFixed(2)}</span>
    },
    {
      header: "Price",
      accessor: "sellingPrice",
      render: (row) => <span className="text-sm font-bold text-gray-900 font-mono">GH₵{Number(row.sellingPrice || 0).toFixed(2)}</span>
    },
    {
      header: "Status",
      accessor: "isActive",
      render: (row) => (
        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${
          row.isActive ? "bg-green-50 text-green-700 border-green-100" : "bg-gray-50 text-gray-500 border-gray-200"
        }`}>
  ...
  return (
    <div className="min-h-screen bg-gray-50/50">
      ...
      <Modal isOpen={showAddModal} onClose={() => setShowAddModal(false)} title="Add Product" size="md">
        <div className="p-1">
          <MaterialForm
            onSubmit={handleAdd}
      ...
      <Modal isOpen={showBulkReorderModal} onClose={() => setShowBulkReorderModal(false)} title="Bulk Restock" size="xl">
        <div className="p-1">
          <BulkReorderModal
