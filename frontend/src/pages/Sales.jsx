...
export default function Sales() {
  ...
  const topUpMutation = useMutation({
    ...
    onError: (error) => alert(error.response?.data?.error || "Failed to process payment top-up"),
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => alert(error.response?.data?.error || "Failed to update sale"),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error) => alert(error.response?.data?.error || "Failed to delete sale"),
  });

  const handleTopUp = (formData) => topUpMutation.mutate({ id: selectedSale.id, data: formData });
  const handleEditSale = (formData) => updateMutation.mutate({ id: selectedSale.id, data: formData });
  const handleDeleteSale = (formData) => deleteMutation.mutate({ id: selectedSale.id, data: formData });
  ...
  const openTopUpModal = (sale) => { setSelectedSale(sale); setShowTopUpModal(true); };
  ...
  const handleCloseReceiptModal = () => {
    ...
    setTimeout(() => { setSelectedReceipt(null); setSelectedSale(null); }, 300);
  };
  ...
  useEffect(() => { setPage(1); }, [dateRange, paymentMethodFilter, paymentStatusFilter]);
  ...
  const columns = useMemo(() => [
    {
      ...
      render: (row) => (
        <div className="flex flex-col">
          <span className="font-medium text-gray-900 text-sm">{format(new Date(row.saleDate), "MMM dd, yyyy")}</span>
          <span className="text-[11px] text-gray-400">{format(new Date(row.saleDate), "HH:mm")}</span>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col">
          <span className="font-bold text-gray-900 text-sm">GH₵{parseFloat(row.totalAmount).toFixed(2)}</span>
          {row.amountPaid < row.totalAmount && (
             <span className="text-[10px] text-gray-500 font-medium">Paid: {parseFloat(row.amountPaid).toFixed(2)}</span>
          )}
        </div>
      ),
    },
    {
      ...
      render: (row) => (
        <span className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-md border ${
            row.paymentStatus === "paid" ? "bg-gray-100 text-gray-600 border-gray-200" : 
            ["partial", "partially"].includes(row.paymentStatus) ? "bg-gray-900 text-white border-gray-900 shadow-sm" : 
            "bg-white text-red-600 border-red-200"
          }`}>
          {row.paymentStatus === "paid" ? "PAID" : ["partial", "partially"].includes(row.paymentStatus) ? "PARTIAL" : row.paymentStatus}
        </span>
      ),
    },
    {
    ...
    },
    {
        ...
        render: (row) => (
          <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-600">{row.user?.fullName || row.user?.username || "N/A"}</span>
          </div>
        ),
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-1">
          {["partial", "partially"].includes(row.paymentStatus) && hasPermission("sales", "create") && (
            <button onClick={() => openTopUpModal(row)} className="p-1.5 text-gray-900 hover:bg-gray-100 rounded-md transition-all border border-transparent hover:border-gray-200" title="Add Payment">
          ...
          <button onClick={() => viewReceipt(row)} className="p-1.5 text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-md transition-all" title="View Receipt">
          ...
          {hasPermission("sales", "update") && (
            <button onClick={() => openEditModal(row)} className="p-1.5 text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-md transition-all" title="Edit Sale">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
            </button>
          )}
          {hasPermission("sales", "delete") && (
            <button onClick={() => openDeleteModal(row)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-md transition-all" title="Delete Sale">
    ...
    <div className="min-h-screen bg-gray-50/50">
      ...
      <div className={`container mx-auto ${RESPONSIVE_SPACING.container} ${RESPONSIVE_SPACING.section} ${showReceiptModal || showEditModal || showDeleteModal || showTopUpModal ? "blur-sm opacity-50 pointer-events-none" : ""}`}>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          {isLoading ? (
            <div className="py-20 flex justify-center"><LoadingSpinner size="lg" text="Loading data..." /></div>
          ) : sales.length === 0 ? (
            <EmptyState icon={<svg className="w-12 h-12 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" /></svg>} title="No sales found" description="Adjust filters or create a new sale." action={hasPermission("sales", "create") ? () => navigate('/sales/new') : null} actionText="Create Sale" />
          ) : (
            <>
              <div className="hidden md:block"><Table columns={columns} data={sales} /></div>
              <div className="md:hidden space-y-3 p-4 bg-gray-50">
                {sales.map((row) => (
                  <div key={row.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col gap-3">
                    {/* ... Mobile view content identical to previous code ... */}
                    <div className="flex justify-between items-start">
                        <div><p className="font-bold text-gray-900">{format(new Date(row.saleDate), "MMM dd, yyyy")}</p><p className="text-xs text-gray-400">{format(new Date(row.saleDate), "hh:mm a")}</p></div>
                        <div className="flex flex-col items-end gap-1"><span className="font-mono text-[10px] bg-gray-50 text-gray-500 border border-gray-100 px-1.5 py-0.5 rounded">#{row.receipt?.receiptNumber || "N/A"}</span><span className={`text-[10px] uppercase font-bold px-1.5 py-0.5 rounded-sm ${row.paymentStatus === 'paid' ? 'bg-gray-100 text-gray-600' : 'bg-gray-900 text-white'}`}>{row.paymentStatus === 'paid' ? 'PAID' : 'PARTIAL'}</span></div>
                    </div>
                    <div className="border-t border-gray-50" />
                    <div className="flex justify-between items-center">
                        <div className="flex flex-col"><span className="text-lg font-bold text-gray-900 tracking-tight">GH₵{parseFloat(row.totalAmount).toFixed(2)}</span><span className="text-[10px] uppercase tracking-wider text-gray-500 font-medium">{row.paymentMethod}</span></div>
                        <div className="flex items-center gap-1">
                            {["partial", "partially"].includes(row.paymentStatus) && hasPermission("sales", "create") && (<button onClick={() => openTopUpModal(row)} className="p-2 bg-gray-50 text-gray-900 border border-gray-200 rounded-lg"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg></button>)}
                            <button onClick={() => viewReceipt(row)} className="p-2 text-gray-400 hover:text-gray-900"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg></button>
                        </div>
                    </div>
                  </div>
                ))}
      ...
      {showTopUpModal && selectedSale && (
        <TopUpModal isOpen={showTopUpModal} onClose={() => { setShowTopUpModal(false); setSelectedSale(null); }} sale={selectedSale} onSubmit={handleTopUp} isSubmitting={topUpMutation.isPending} />
      )}
      {selectedSale && (
        <EditSaleModal isOpen={showEditModal} onClose={() => { setShowEditModal(false); setSelectedSale(null); }} sale={selectedSale} onSubmit={handleEditSale} isLoading={updateMutation.isPending} />
      )}
      {selectedSale && (
        <DeleteSaleModal isOpen={showDeleteModal} onClose={() => { setShowDeleteModal(false); setSelectedSale(null); }} sale={selectedSale} onConfirm={handleDeleteSale} isLoading={deleteMutation.isPending} />
      )}
      {selectedReceipt && selectedSale && (
        <Modal isOpen={showReceiptModal} onClose={handleCloseReceiptModal} title="Sale Receipt" size="md">
          <div className="p-1"><Receipt receipt={selectedReceipt} sale={selectedSale} /></div>
        </Modal>
      )}
    </div>
  );
